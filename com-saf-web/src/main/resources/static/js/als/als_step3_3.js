function init_step3_form() {
    layui.use('form', function () {
        ...
        $('#button_variance_setting').bind('click', function () {
            ...
            http_utils.postData(monitor_path + '/mllib/als/execute', JSON.stringify($(form_id).serializeJSON()), true, function (data) {
                if (data) {
                    if (data.status == 200) {
...
function reload_step3_ui(is_reload) {
    if (is_reload) {
        saf_layui.loding_increase('preview_step3_als', 1);
    } else {
        saf_layui.loding_increase('preview_step3_als', 0);
    }
}
...
function showResponse_step3(message) {
    if (message.responseMessageType == 'progress') {
        saf_layui.loding_increase('preview_step3_als', message.responseMessage);
        ...
        if (message.responseMessageFormat == 'json') {
            ...
            if (json.type == 1) {
                if (json.data) {
                    ...
                    var rank_min_max = '(' + $('#als_rank_min').val() + '/' + $('#als_rank_max').val() + ')';
                    var lambda_min_max = '(' + $('#als_lambda_min').val() + '/' + $('#als_lambda_max').val() + ')';
                    var alpha_min_max = '(' + $('#als_alpha_min').val() + '/' + $('#als_alpha_max').val() + ')';
                    var iter_min_max = '(' + $('#als_iter_min').val() + '/' + $('#als_iter_max').val() + ')';
                    ...
                    if (data.isBest) {
                        ...
                        $('#als_best_rank').attr('value', data.rank);
                        $('#als_best_lambda').attr('value', data.lambda);
                        $('#als_best_alpha').attr('value', data.alpha);
                        $('#als_best_iter').attr('value', data.iter);
